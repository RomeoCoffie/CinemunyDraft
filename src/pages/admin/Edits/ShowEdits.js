...
console.log(dirOption,'option')
  ...
  let theShow=state.showDetails
  console.log(theShow,'thisShow')

  let submitNewTitle= async (newTitle)=>{
    const theRef = doc(db, 'shows', theShow.id);
  ...
  const handleFileChange = async (e) => {
    ...
    if (selected) {
      ...
      const metadata = {
        ...
        customMetadata: {
          title: theShow.title,
          description: theShow.desc,
          year: theShow.year,
        },
      };

      const storageRef = ref(storage, `/shows/${theShow.title}/${selected.name}`);
  ...
  let submitNewSeasons= async (newSeasons)=>{
    const theRef = doc(db, 'shows', theShow.id);
    console.log(newSeasons,'theNewDesc')
    if (newSeasons==='' || newSeasons===undefined) {
      alert('Please input a new Title')
      return;
    }
    try {
  ...
  let editBlock=(whereEdit)=>{
    ...
    if (whereEdit==='keywords') {
    ...
    } else if (whereEdit==='seasons'){
      return (
        <div>
        <div>
            Seasons: {theShow.seasons}
          </div>
            <input type="number"
             onChange={(e)=>{setNewSeasons(e.target.value)}} placeholder='Number of Seasons' />
            <button onClick={(e)=>{submitNewSeasons(newSeasons)}}>Submit New Rating</button>
        </div>)
      ...
      return (<div>
        <div><img src={theShow.movieImgUrl} alt='' /></div>
        <div>
        <input
                type="file"
      ...
      <div className='editWhere'>
        <label htmlFor='editTypes'>Where do you want to make the edit?</label>
          ...
          <option value={null}>Choose...</option>
            ...
            <option value='seasons'>Number of Seasons</option>
